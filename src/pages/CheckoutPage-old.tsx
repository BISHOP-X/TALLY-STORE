...
import { CheckCircle, CreditCard, Wallet, ArrowLeft, Shield, Clock, Loader2 } from 'lucide-react'
...
export default function CheckoutPage() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const result = await processPurchase(user.id, product.id)
      ...
      if (result.success) {
        ...
        // Redirect to orders with purchase success and order data
        navigate(`/orders?purchase=success&order=${result.orderData?.id}`)
      } else {
        toast({
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">{product.categories?.name} Account</p>
                    <div className="flex gap-2 mt-2">
                      <Badge variant="outline">Stock: {product.stock_count}</Badge>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  {product.features && product.features.length > 0 ? (
                    ...
                    <div className="flex items-center gap-2">
                      <CheckCircle className="h-4 w-4 text-green-500" />
                      <span className="text-sm">Complete account access</span>
                    </div>
                  )}
                  <div className="flex items-center gap-2">
